...
function ReviewForm() {
    ...
    const handleSubmit = async (wind) => {
        wind.preventDefault();

        if (!name || !email || !reviewText || rating === 0 || !agree) {
            setAlertMessage({ text: 'Заполните все обязательные поля!', type: 'error' });
            return;
        }
        ...
        try {
            const response = await fetch('https://6756c695c0a427baf94a5624.mockapi.io/revievse', {
                method: 'POST',
            ...
            if (response.ok) {
                ...
                const updatedReviews = await fetch('https://6756c695c0a427baf94a5624.mockapi.io/revievse')
                    .then(response => response.json());
                setReviews(updatedReviews);
            }
        } catch (error) {
            console.error('Ошибка:', error);
            setAlertMessage({ text: 'Произошла ошибка при отправке отзыва.', type: 'error' });
        }
    };

    const handleDelete = async (reviewId) => {
        try {
            const response = await fetch(`https://6756c695c0a427baf94a5624.mockapi.io/revievse/${reviewId}`, {
                method: 'DELETE',
            });
            ...
            console.error('Ошибка:', error);
            setAlertMessage({ text: 'Произошла ошибка при удалении отзыва.', type: 'error' });
        }
    };
    ...
    return (
        <div className="container">
            {alertMessage && (
                <div className={`alert ${alertMessage.type === 'error' ? 'error' : ''}`}>
                    <span>{alertMessage.text}</span>
                    <button className="alert__button" onClick={() => setAlertMessage(null)}>&times;</button>
                </div>
            )}

            <div className="revievs__title">
                <h1 className="revievs__title-text">
                    Оставьте свой отзыв
                </h1>
            </div>
                ...
                <form id="revievsForm" onSubmit={handleSubmit}>
                    <div className="revievs__forms animate-left">
                        ...
                        <div className="revievs__rate">
                            <p className="revievs__rate-text">
                                Ваша оценка 
                            </p>
                        ...
                        <div className="revievs__reviev">
                            <p className="revievs__reviev-text">
                                Оставьте отзыв
                            </p>
