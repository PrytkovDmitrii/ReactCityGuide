...
function Header() {
  ...
  useEffect(() => {
    const elements = [document.body, document.getElementById('footer'), document.getElementById('header')];
    elements.forEach((element) => {
      if (element) {
        isDarkTheme ? element.classList.add('black') : element.classList.remove('black');
      }
    });
  ...
  return (
    <header className={`header ${isDarkTheme ? 'black' : ''} ${isMenuOpen ? 'open' : ''}`} id="header">
      <div className="container" id="line">
        <div className="header__wrap">
          <button className="header__burger-btn" id="burger" onClick={toggleMenu}>
            <span></span>
            <span></span>
            <span></span>
          </button>
          ...
          <div className="header__menu animate-down">
            <HeaderLink className="header__text" icon={newspaperIcon} title="Главная" href='/' />
            <HeaderLink className="header__text" icon={attractIcon} title="Достопримечательности" href='/attract' />
            <HeaderLink className="header__text" icon={mapIcon} title="Карты" href='/maps' />
            <HeaderLink className="header__text" icon={contactIcon} title="Контакты" href='/contacts' />
